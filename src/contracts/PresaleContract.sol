...
contract BNBRFPresale is Ownable, ReentrancyGuard {
   ...
   event TokensWithdrawn(address indexed to, uint256 amount);
    ...
    require(_minReferralPayoutBnb > 0,    "Bnb min payout must be > 0");
        ...
        // stage data
        stages[1] = Stage(1, 50, 3000000000000000, 1752261600, 1753212000);    // 0.003
        stages[2] = Stage(2, 35, 5000000000000000, 1753212001, 1753471200);    // 0.005
        stages[3] = Stage(3, 30, 6000000000000000, 1753471201, 1753644000);    // 0.006
        stages[4] = Stage(4, 22, 7000000000000000, 1753644001, 1753730400);    // 0.007
        stages[5] = Stage(5, 15, 8000000000000000, 1753730401, 1754076000);    // 0.008
        stages[6] = Stage(6, 12, 9000000000000000, 1754076001, 1754248800);    // 0.009
        stages[7] = Stage(7, 10, 10000000000000000, 1754248801, 1754508000);   // 0.010
        stages[8] = Stage(8, 10, 12000000000000000, 1754508001, 1754767200);   // 0.012
        stages[9] = Stage(9, 8, 15000000000000000, 1754767201, 1755285600);    // 0.015
        currentStageId = 9;       
    ...
    // buyToken function to buy tokens - Fixed for Checks-Effects-Interactions
    function buyToken(address _referrer) public payable nonReentrant {          
    ...
    // Calculate USD value of sent ETH/BNB
    uint256 usdValue = weiToUsd(msg.value);
    ...
    uint256 newCount = 0;
    uint256 referralPct = 0;
    if (_referrer != address(0) && _referrer != msg.sender) {
        uint256 currentCount = referralCounts[_referrer];
        newCount = currentCount + 1;
        referralPct = getReferralBonusPercentage(newCount);
        if (referralPct > 0) {
            referralCashback = (msg.value * referralPct) / 100;
        }
    }
    ...
    if (referralCashback > 0) {
        referralCounts[_referrer] = newCount;
    ...
    // Send payment to payment address (minus referral cashback)
    uint256 paymentToAddress = msg.value - referralCashback;
    (bool sent, ) = payable(paymentAddress).call{ value: paymentToAddress }("");
    require(sent, "Payment transfer failed");    

    // 4) EVENTS - Emit after all interactions
    if (referralCashback > 0) {
        emit ReferralPurchase(_referrer, msg.sender, usdValue, msg.value, referralCashback, referralPct, newCount);
    } 
    }  
   ...
   function calculateRequiredPayment(uint256 tokenAmount) public view returns (uint256) {
    ...
    // Interaction - Send ETH/BNB to referrer
   ...
   function getReferrerInfo(address _referrer) external view returns (
    ...
    function emergencyWithdrawReferralFunds() external onlyOwner nonReentrant {
    uint256 balance = address(this).balance;
